...
import { formatPrice, formatCountdown } from '../utils/haat';
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    getOneOfOneProducts().then((p) => setOneOfOne(p.slice(0, 5)));
    getSmallBatchProducts().then((p) => setSmallBatch(p.slice(0, 8)));
  ...
  return (
    <main>
      {/* ═══ 01 HERO — Full-bleed haat photograph ═══ */}
      <section
        className="photo-bleed photo-bleed--tall"
      ...
      >
        ...
        <div className="photo-content">
          <div className="container">
            ...
            <h1 className="display-xl" style={{ marginBottom: 16, maxWidth: 700 }}>
              হাট বসেছে।
            </h1>
      ...
      {/* ═══ 02 FEATURED STALL — physical haat stall metaphor ═══ */}
      {feature && (
            ...
            <div className="stall-feature">
              ...
              <div className="stall-feature-panel">
                <div style={{ marginBottom: 8 }}>
                  {feature.productType === 'one-of-one' && (
                    <span className="badge badge-quantity" style={{ marginRight: 6 }}>মাত্র ১টি</span>
                  )}
                ...
                <p className="caption" style={{ marginBottom: 16 }}>
                  {[feature.creator?.name, feature.origin].filter(Boolean).join(' · ')}
                </p>
                <div style={{ marginBottom: 8 }}>
                  <div style={{ fontSize: 11, fontFamily: 'var(--font-en)', letterSpacing: '0.06em', color: 'var(--muted)', marginBottom: 4 }}>
                    হাটের দাম
                  </div>
                ...
                <div style={{ marginTop: 16, marginBottom: 24 }}>
                  <span className="countdown" style={{ color: 'var(--haat-accent)', fontSize: 14 }}>
                    {remaining.h}:{remaining.m}:{remaining.s} বাকি
                  </span>
                </div>
                <Link to={`/product/${feature.slug}`} className="btn btn-primary" style={{ alignSelf: 'flex-start' }}>
                  দেখুন
                </Link>
              </div>
            </div>
      ...
      {/* ═══ 03 WALKING PAST STALLS — horizontal haat carousel ═══ */}
      <section style={{ background: 'var(--warm-white)', padding: '48px 0 32px' }}>
        ...
        <div className="stall-scroll">
          {haatProducts.map((p) => (
            <ProductCard key={p.id} product={p} variant="tag" size="md" />
          ))}
        </div>
        <div className="container" style={{ paddingTop: 8 }}>
          <Link to="/friday-haat" style={{ fontFamily: 'var(--font-heading)', fontWeight: 600, fontSize: 15, borderBottom: '1px solid var(--ink)' }}>
            পুরো হাট দেখুন →
          </Link>
        </div>
      </section>

      {/* ═══ 04 ONE OF ONE — dramatic editorial ═══ */}
      <section
        className="photo-bleed photo-bleed--mid"
        style={{ backgroundImage: `url(${IMG.ceramics})` }}
      >
        ...
        <div className="photo-content">
          <div className="container">
            ...
            <p className="lead" style={{ maxWidth: 360, marginBottom: 28 }}>
              একইভাবে আরেকটা বানানো হবে না।
            </p>
            <Link to="/explore?type=one-of-one" className="btn btn-secondary">
              একক সৃষ্টি দেখুন
            </Link>
          </div>
        </div>
      ...
      {/* One-of-one products — editorial grid */}
      {oneOfOne.length > 0 && (
        <section className="section" style={{ background: 'var(--cream)', paddingTop: 48 }}>
          <div className="container">
            <div className="oneof-grid">
              {oneOfOne[0] && (
                <div className="oneof-featured">
                  <ProductCard product={oneOfOne[0]} variant="editorial" />
                </div>
              )}
              <div className="oneof-side">
                {oneOfOne.slice(1, 4).map((p) => (
      ...
      {/* ═══ 05 CREATOR STORY — full-bleed workshop ═══ */}
      {featuredCreator && (
        <section
          className="photo-bleed photo-bleed--tall"
          ...
          <div className="photo-content">
            <div className="container">
              ...
              <p style={{ opacity: 0.75, marginBottom: 24, fontSize: 14 }}>
                {featuredCreator.location} · {featuredCreator.specialty}
              </p>
              ...
              <Link to={`/creator/${featuredCreator.id}`} className="btn btn-secondary">
                তার কাজ দেখুন
              </Link>
            </div>
          </div>
      ...
      {/* ═══ 06 JOURNEY — village to home ═══ */}
      <section
        className="photo-bleed photo-bleed--mid"
        style={{ backgroundImage: `url(${IMG.rural})` }}
      >
      ...
      <section style={{ background: 'var(--ink)', padding: '64px 0 0' }}>
        <div className="container" style={{ marginBottom: 32 }}>
          ...
          <h2 className="display-md" style={{ color: 'white', marginBottom: 8 }}>
            পুরোনো কাজ। নতুন করে দেখা।
          </h2>
          <p style={{ color: 'rgba(255,255,255,0.65)', maxWidth: 400 }}>
            বাংলার পুরোনো কারুশিল্প আজকের ঘরে নতুনভাবে ফিরে আসছে।
          </p>
        </div>
      ...
      <section className="section" style={{ background: 'var(--cream)' }}>
        <div className="container">
          ...
          <p className="text-muted" style={{ marginBottom: 32, maxWidth: 360 }}>
            ১০০টা নয়। ৫টা হলেও যথেষ্ট।
          </p>
          <div className="stall-scroll stall-scroll--inset">
            {smallBatch.map((p) => (
              <ProductCard key={p.id} product={p} variant="tag" size="md" />
            ))}
          </div>
      ...
      {/* ═══ 10 DEEP GREEN BRAND STATEMENT ═══ */}
      <section className="section-green">
        <div className="container" style={{ textAlign: 'center' }}>
          <h2 className="display-lg" style={{ marginBottom: 16 }}>
            যা সবাই বানায় না।
          </h2>
          <p style={{ fontSize: 18, opacity: 0.85, maxWidth: 400, margin: '0 auto 40px' }}>
            তাই LOCAL-এ আসে।
          </p>
          ...
          <Link to="/explore" className="btn btn-on-green">
            অন্বেষণ করুন
          </Link>
        </div>
      </section>
      ...
      <section className="section" style={{ background: 'var(--warm-white)' }}>
        <div className="container">
          ...
          <h2 className="display-md" style={{ marginBottom: 8 }}>
            বাংলাদেশ শুধু ঐতিহ্য বানায় না।
          </h2>
          ...
          <div style={{ marginTop: 28 }}>
            <Link to="/creators" className="btn btn-secondary-dark">
              সব নির্মাতা
            </Link>
          </div>
        </div>
      ...
      {/* ═══ 12 BECOME CREATOR CTA ═══ */}
      <section
        className="photo-bleed photo-bleed--short"
        style={{ backgroundImage: `url(${IMG.handsClay})` }}
      >
        <div className="photo-overlay" />
        <div className="photo-content" style={{ textAlign: 'center' }}>
          <div className="container">
            ...
            <Link to="/become-a-creator" className="btn btn-haat">
